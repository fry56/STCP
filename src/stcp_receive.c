...
void stcp_init_header()
{
    stcp_datas->packet->header = *(stcp_packet_header *)binary_to_str(stcp_datas->buffer);
    if (stcp_datas->packet->header.hash != PACKET_HASH) {
        printf("Corrupted packet drop. (HASH : %d)\n", stcp_datas->packet->header.hash);
        stcp_destroy_packet();
        stcp_datas->waiting_data = true;
    }
    printf("Valide Header. (HASH : %d)\n", stcp_datas->packet->header.hash);
}

void stpc_init_packet()
{
    ...
    stcp_datas->waiting_data = false;
}

void stcp_call_packet_function()
{
...
void stcp_check_packet()
{
    printf("--- Packet receive ---\n");
    printf("Type : %d \n", stcp_datas->packet->header.type);
    printf("Size : %d \n", stcp_datas->packet->header.size);
    if (stcp_datas->packet->header.type != 1 && stcp_datas->linked_puid == -1
        && !stcp_datas->waiting_hand_check) {
